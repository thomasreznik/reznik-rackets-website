...
<table class= "table table-hover table-bordered" id = "strings_table" >
    ...
    <tr class="table-dark">
        ...
        <th scope="col" class="text-center" style="font-size:25px">Gauge</th>
...
//Query to get all string products from the particular brand that the user selected

$product_sql = "SELECT * FROM Strings WHERE Brand_ID = $brand_id";
...
if ($result->num_rows > 0) {
    ...
    //Creating a table within Gauge Column to display gauge vertically
        $modify_string_gauge = explode(" ", $row[3]);
        $mod_string_gauge_str = "<table>";

        foreach ($modify_string_gauge as &$line) {
            $mod_string_gauge_str .= "<tr><td align='center'>$line</td></tr>";
        }
        $mod_string_gauge_str .= "</table>";
        
    //Creating a table within Gauge Column to display gauge vertically
        $modify_string_color = explode(" ", $row[4]);
        $mod_string_color_str = "<table>";

        foreach ($modify_string_color as &$line) {
            $mod_string_color_str .= "<tr><td align='center' >$line</td></tr>";
        }
        $mod_string_color_str .= "</table>";    
    ...
    $myDisplayResults .= "
      ...
      <td align='center' style='font-size:20px'>$mod_string_gauge_str</td>
      <td align='center' style='font-size:20px'>$mod_string_color_str</td>
